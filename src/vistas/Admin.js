...
export const admin = {
    template: `<div class="ms-5 mt-5 row justify-content-between">
    ...
    script: ()=>{
        ...
        for(let i = 0; i< adminUsuarios.array.length; i++){
            ...
            fila += `<tr id="${i}">
                        ...
                        <td><button type="button" class="btn btn-success editar me-3" id="editar" data-id = "${adminUsuarios.array[i].id}" data-bs-toggle="modal" data-bs-target="#modal">Editar</button><button type="button" class="btn btn-danger eliminar" data-id = "${adminUsuarios.array[i].id}">Eliminar</button></td>
                    </tr>`
        }
        ...
        document.getElementById('boton-enviar').addEventListener('click', (e)=>{
            ...
            let fila= `<tr>
                            ...
                            <td>${email}</td>
                            <td><button type="button" class="btn btn-success editar" data-id = "${objeto.id}"  data-bs-toggle="modal" data-bs-target="#modal">Editar</button><button type="button" class="btn btn-danger eliminar" data-id = "${objeto.id}">Eliminar</button></td>
                        </tr>`
